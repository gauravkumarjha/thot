...
namespace DiMedia\ShippingDayUnicomerce\Observer;
...
class SaveCustomProductAttribute implements ObserverInterface
{
    ...
    {
        // Initialize logger
        $writer = new Zend_Log_Writer_Stream(BP . '/var/log/ShippingDayUnicomerce.log');
        $logger = new Zend_Log();
        ...
        foreach ($order->getAllItems() as $item) {
          ...
          //  $product->setStoreId($storeId);
            ...
            $extraDays = $this->shippingHelper->getShippingDays();
           // $extraDays = ($storeCode == 'usd') ? 12 : 6;
            ...
            //$fulfillment_date = $product->getData('fulfillment_date');

            if ($fulfillmentDate) {
                $item->setData('fulfillment_date', $fulfillmentDate);
               // $item->save(); // important, since order is already saved
            }
