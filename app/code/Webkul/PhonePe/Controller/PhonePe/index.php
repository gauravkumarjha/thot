    ...
    public function execute()
    {
        ...
        try {
            ...
            if (!$quote->getId() || !$quote->getAllVisibleItems()) {
                throw new \Exception('Quote not found or has no items.');
            }

            // ✅ STEP 1: Create order before payment
            $this->logger->info('Creating pre-payment order for Quote ID: ' . $quote->getId());
            $quoteId= $quote->getId();
            ...
            if (!$order || !$order->getId()) {
                $this->logger->critical('Quote submit returned NULL. QuoteID=' . $quote->getId());
                throw new \Exception('Quote submission returned null (order not created)');
            }

            $this->logger->info('✅ Order created successfully: OrderID=' . $order->getEntityId() . ', IncrementID=' . $order->getIncrementId());
          
            $order->setState(\Magento\Sales\Model\Order::STATE_PENDING_PAYMENT)
                ->setStatus('pending_payment');
            $this->orderRepository->save($order);
            $merchantTransactionId = "ORDER_" . time() . '_' . $order->getIncrementId();
            $transactionId = $this->helper->encryptData($merchantTransactionId);
            $transactionId2 = $this->helper->decryptData($transactionId);
            $quoteId = $this->helper->encryptData($order->getEntityId());
           ...
           $queryParams = [
            ...
            $this->logger->info('phone pe payment initialize response  :- transactionId -'. $transactionId . ' transactionId2-'. $transactionId2 .json_encode($payResponse->jsonSerialize()));
