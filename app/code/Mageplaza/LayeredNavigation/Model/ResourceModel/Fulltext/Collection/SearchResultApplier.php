...
use Mageplaza\LayeredNavigation\Model\StockStatusApplierInterface;
...
class SearchResultApplier implements SearchResultApplierInterface
{
    ...
    private function categoryProductByCustomSortOrder(int $categoryId): array
    {
        ...
        $productIds = array_map(fn($item) => $item->getId(), $this->searchResult->getItems());
        $query->where('e.entity_id IN(?)', $productIds);

        $eavAttributeTable = $this->collection->getTable('eav_attribute');
        echo "Gaurav1";
        die();
        foreach ($sortOrders as $field => $dir) {
            if ($field === 'name') {
                ...
                $query->joinLeft(
                    ...
                    (SELECT attribute_id FROM {$eavAttributeTable} WHERE entity_type_id={$entityTypeId}
