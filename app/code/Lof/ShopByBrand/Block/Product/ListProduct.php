 ...
 */
namespace Lof\ShopByBrand\Block\Product;
...
class ListProduct extends \Magento\Catalog\Block\Product\AbstractProduct implements IdentityInterface
{
    ...
    /* protected function _prepareLayout()
    {
        ...
        $block = $this->getLayout()->getBlock('catalog.navigation.state');
        if ($nav = $this->getLayout()->getBlock('catalog.leftnav')) {
            $_filters = $nav->getFilters();
            foreach ($_filters as $key => $_filter) {
                if ($_filter->getRequestVar() == 'manufacturer') {
                    unset($_filters[$key]);
                    $nav->setFilters($_filters);
                    break;
                }
            }
        }
        //die();
        $block = $this->getLayout()->getBlock('catalog.navigation.state');
        echo count($this->getLayer()->getState()->getFilters()); die();
