...
use Olegnax\HotSpotQuickview\Block\Index\Index;
...
class HotSpot extends Template implements BlockInterface
{
    ...
    public function getCustomHref()
    {
        if ($this->_custom_href === null) {
            $id = $this->getData('modal_custom_content');
            if ($id) {
                $href = $this->urlInterface->getUrl('ox_hotspot/index/index',
                    [Index::AJAX_ATTR => $this->getData('modal_custom_content')]);

                $this->_custom_href = $href;
            }
        }
        return $this->_custom_href;
    }
