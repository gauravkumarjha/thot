...
use Magento\Checkout\Model\Cart as ParentCart;
...
class Cart extends ParentCart
{
     ...
     * @param CheckoutCart $resourceCart
    ...
    protected function reinitializeState()
    {
        ...
        if ($this->_checkoutSession->getCheckoutState() !== CheckoutSession::CHECKOUT_STATE_BEGIN
    ...
    public function updateItems($data)
    {
        ...
        foreach ($data as $itemId => $itemInfo) {
            ...
            if ($qty > 0) {
                $item->setQty($qty);
                if ($item->getProductType() == 'giftcertificate' && $item->getQty() >1) {
                    $item->setQty(1);
