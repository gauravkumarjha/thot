...
class Collection extends \Magento\Catalog\Model\ResourceModel\Product\Collection
{
    ...
    public function getGroupedProducts($storeId, $notLike, $concat, $listOfAttributes)
    {
        ...
        $this->getSelect()->joinLeft(['cpl' => $tableCpl], 'cpl.product_id=e.' . $this->_rowId . ' AND cpl.link_type_id=3', ['child_ids' => 'GROUP_CONCAT(DISTINCT cpl.linked_product_id)'])->joinLeft(['stock' => $tableCsi], 'stock.product_id=e.entity_id', ['qty' => 'qty', 'is_in_stock' => 'is_in_stock', 'manage_stock' => 'manage_stock', 'use_config_manage_stock' => 'use_config_manage_stock', 'backorders' => 'backorders', 'use_config_backorders' => 'use_config_backorders'])->joinLeft(['url' => $tableCur], "url.store_id=" . $storeId . $notLike, ["request_path" => $concat . "(DISTINCT request_path)"])->joinLeft(['categories' => $tableCcp], "categories.product_id=e.entity_id", ["categories_ids" => "GROUP_CONCAT( DISTINCT categories.category_id)"])->group(['cpl.product_id']);
        $grouped = [];
    ...
    protected function getParentProducts($type, $storeId, $notLike, $concat, $listOfAttributes)
    {
        ...
        $this->getSelect()->joinLeft(['cpsl' => $tableCpsl], 'cpsl.parent_id=e.' . $this->_rowId, ['child_ids' => 'GROUP_CONCAT( DISTINCT cpsl.product_id)'])->joinLeft(['stock' => $tableCsi], 'stock.product_id=e.entity_id', ['qty' => 'qty', 'is_in_stock' => 'is_in_stock', 'manage_stock' => 'manage_stock', 'use_config_manage_stock' => 'use_config_manage_stock', 'backorders' => 'backorders', 'use_config_backorders' => 'use_config_backorders'])->joinLeft(['url' => $tableCur], "url.store_id=" . $storeId . $notLike, ["request_path" => $concat . "(DISTINCT request_path)"])->joinLeft(['curpc' => $tableCurpc], "url.url_rewrite_id=curpc.url_rewrite_id ")->joinLeft(['categories' => $tableCcp], "categories.product_id=e.entity_id", ["categories_ids" => "GROUP_CONCAT( DISTINCT categories.category_id)"])->group(['cpsl.parent_id']);
        $parent = [];
        foreach ($this as $p) {
