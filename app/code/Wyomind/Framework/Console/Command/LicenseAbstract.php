...
class LicenseAbstract extends Command
{
    ...
    protected function activate($module, & $input, & $output, $ak = false, $autoRequest = false)
    {
        ...
        $secureUrl = $this->deploymentConfig->get("system/default/web/secure_url");
        $unsecureUrl = $this->deploymentConfig->get("system/default/web/unsecure_url");
        if ($secureUrl == "") {
            $secureUrl = $this->config->getDefaultValueByPath("web/secure_url");
        }
        if ($unsecureUrl == "") {
            $unsecureUrl = $this->config->getDefaultValueByPath("web/unsecure_url");
        }
        if ($secureUrl == "") {
