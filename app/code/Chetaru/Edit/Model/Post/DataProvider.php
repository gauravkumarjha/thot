...
namespace Chetaru\Edit\Model\Post;
...
class DataProvider extends ModifierPoolDataProvider
{
    ...
    public function getData(): array
    {
        ...
        foreach ($items as $model) {
            ...
            if ($model->getLogo()) {
                $m[0]['name'] = basename($model->getLogo());
                $m[0]['url'] = $model->getLogo();
                $this->loadedData['logo'] = $m;
            }
            if ($model->getBannerImage()) {
                $m[0]['name'] = basename($model->getBannerImage());
                $m[0]['url'] = $model->getBannerImage();
                $this->loadedData['banner_image'] = $m;
            }
            if ($model->getBrandPageImage()) {
                $m[0]['name'] = basename($model->getBrandPageImage());
                $m[0]['url'] = $model->getBrandPageImage();
                $this->loadedData['brand_page_image'] = $m;
            }
            if ($model->getCategories()) {
                $m = json_decode($model->getData('categories'));
                $this->loadedData['categories'] = $m;
            }
