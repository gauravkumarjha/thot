    ...
    protected function _getProductCollection()
    {
        if ($this->_productCollection === null) {
            ...
            $select = $connection->select()->from(
                $resource->getTable('ves_brand_product'),
                '*'
            )
            ->where(
                'brand_id = ?',
           ...
           /// print_r($result );
            ...
            $collection->addAttributeToSelect('*')->addFieldToFilter('visibility', ['neq' => 1]);
            ///die();
            $collection->addAttributeToFilter('entity_id', ['in' => $result]);
