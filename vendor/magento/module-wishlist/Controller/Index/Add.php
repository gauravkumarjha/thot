...
class Add extends \Magento\Wishlist\Controller\AbstractIndex implements HttpPostActionInterface
{
    ...
    public function execute()
    {
     ...
     //   print_r($requestParams);
        ...
        // Chetaru customization on link
        $key = 'p';
        $buildnewurl = $this->_redirect->getRefererUrl();
        $buildnewurl = preg_replace('/(?:&|(\?))' . $key . '=[^&]*(?(1)&|)?/i', "$1", $buildnewurl);
        $buildnewurl = rtrim($buildnewurl, '?');
        $buildnewurl = rtrim($buildnewurl, '&');
        $buildnewurl = strtok($buildnewurl, '?');
        //echo $referer;
        //die();
        //die();
        $resultRedirect->setUrl($buildnewurl);
