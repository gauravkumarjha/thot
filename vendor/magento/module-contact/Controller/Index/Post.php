...
class Post extends \Magento\Contact\Controller\Index implements HttpPostActionInterface
{
    ...
    private function myfreshworksUpsert()
    {
        ...
        $query_about = $request->getParam('query_about');
        $f_name = $request->getParam('name');
        ...
        curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://thehouseofthings-org.myfreshworks.com/crm/marketer/mas/api/v1/events?email='.$email.'&event_name=contactForm&event_timestamp='.date("Y-m-d").'T20%3A00%3A00Z',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
